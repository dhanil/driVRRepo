...
//				Debug.Log ("Set center of mass for car" + " " +myTransform.root.name);
	...
	void Update () {
		...
		int remainder = (triggerCounter % 2);
		...
		if (gameObject.tag == "Player") {
			...
			if(Cardboard.SDK.Triggered)
			{
				...
				triggerCounter++;
				print("Trigger pressed::::::::::"+ triggerCounter);
				if(remainder!=0)
				{
					print("Car should move");
					carRigidBody.MovePosition (transform.position + (transform.forward * Time.deltaTime * 3));
				}else{
					print("Car must stop");
					carRigidBody.velocity= (gameObject.transform.position)*0;
				}
			}
			else{
				if(remainder != 0)
...
//		//commented due to not having keyboard control
//		 if (!crashed){
...
//
//	IEnumerator waitForCanvasUI()
//	{
//		//canvasUI.gameObject.SetActive (true);
//		carRigidBody.MovePosition (transform.position + (transform.forward * Time.deltaTime));
//		if (Cardboard.SDK.Triggered) {
//			carRigidBody.velocity= (gameObject.transform.position)*0;
//		}else if(Cardboard.SDK.)
