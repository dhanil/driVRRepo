...
public class TSSimpleCar : MonoBehaviour {
	...
	*/
	...
	void Start () {
		...
		if ((gameObject.tag=="Player") && (canvasIntro.gameObject.activeInHierarchy)) {
			Time.timeScale=0f;
	...
	void OnCollisionEnter(Collision collision)
	{
		foreach (ContactPoint contact in collision.contacts) {
			if(contact.otherCollider.gameObject.transform.parent.name=="car" && gameObject.tag=="Player")
			{
				print ("Car collided!!!!!!!!!!!!!!!"+contact.otherCollider.gameObject.transform.parent.name);
	...
	void Update () {
		...
		if ((gameObject.tag=="Player") && (canvasIntro.gameObject.activeInHierarchy==true) && (Input.GetKey(KeyCode.Escape))) {
			print("game started");
			//camTarget.position= new Vector3(5.2f,0.8f,-8.0f);
			canvasIntro.gameObject.SetActive(false);
			Time.timeScale=1.0f;
		}
		if ((gameObject.tag=="Player") && (Input.GetKey (KeyCode.LeftControl))) {
			Time.timeScale=0;
			canvasPause.gameObject.SetActive(true);
			canvas.gameObject.SetActive(false);
		}
		if ((gameObject.tag=="Player") && (Input.GetKey (KeyCode.Escape))) {
			Time.timeScale=1.0f;
				...
				if (playerControlled) {
					...
					for (int i = 0; i < BackWheels.Length; i++) {
						if (mySpeed < maxSpeed)
							...
							BackWheels [i].motorTorque = -EngineTorque / 5f;
		...
		if (Input.GetKey (KeyCode.Space)&& gameObject.tag=="Player") {
			gameObject.GetComponent<Rigidbody>().velocity = (gameObject.transform.position)*0;
		}
